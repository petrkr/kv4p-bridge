...
class DummyConnector:
    ...
    def open(self, radio: object) -> None:
        ...
        logger.info(
            ...
            "a=TX AFSK diag, o=TX OK1PKR TEST, "
            "7=TX audio cmd 0x07, c=TX audio cmd 0x0c, p=PTT toggle"
        )
        ...
        try:
            ...
            while not self._stop.is_set():
                ...
                if command == "t":
                    ...
                    logger.info("dummy command tx audio command 0x07")
                    self._set_tx_audio_command(0x07)
                elif command == "c":
                    logger.info("dummy command tx audio command 0x0c")
                    self._set_tx_audio_command(0x0C)
                elif command == "p":
                    ptt = not ptt
    ...
    def _set_tx_audio_command(self, command: int) -> None:
        self._radio.set_tx_audio_command(command)
...
def _ax25_ui_frame(source: str, destination: str, digipeaters: list[str], info: bytes) -> bytes:
